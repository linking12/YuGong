...
/**
 * Basic pipeline for pipeline requests. This abstract class can be used for creating custom Pipeline classes.
 *
...
public abstract class AbstractPipeline<R extends PipelineRequest> implements Pipeline<R> {
    ...
    private BlockingQueue<R>    queue;                                                   // = new
    ...
    /**
     * Call this thread. Take next request and pass them to request listeners. At least, this pipeline itself can listen
     * to requests and handle errors. Only PipelineExceptions are handled for each listener. Other execptions will quit
     * the pipeline request executions.
     * 
