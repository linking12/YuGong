 ...
 * the task execution, and scripting.
 */
public class StandardContext<S extends JDBCSource> implements Context<S, Sink> {

    private static final Logger       logger  = LoggerFactory.getLogger("importer.jdbc.context.standard");
    ...
    public StandardContext setSettings(Settings settings) {
        ...
        if (settings.getAsBoolean("metrics.enabled", false) && futures.isEmpty()) {
            ...
            futures.add(scheduledthreadPoolExecutor.scheduleAtFixedRate(thread, 0L,
                                                                        settings.getAsTime("metrics.interval",
                                                                                           TimeValue.timeValueSeconds(30)).seconds(),
                                                                        TimeUnit.SECONDS));
    ...
    @Override
    public StandardContext setSyncTaskDTO(SyncTaskDTO taskDTO) {
        this.syncTaskDTO = taskDTO;
        return this;
    }
    ...
    protected void writeState2() {
        if (source == null || source.getMetric() == null) {
            return;
        }
        try {
            FormatDateTimeFormatter formatter = Joda.forPattern("dateOptionalTime");
            Settings.Builder settingsBuilder = Settings.settingsBuilder().put(settings).put("metrics.lastexecutionstart",
                                                                                            formatter.printer().print(source.getMetric().getLastExecutionStart())).put("metrics.lastexecutionend",
                                                                                                                                                                       formatter.printer().print(source.getMetric().getLastExecutionEnd())).put("metrics.counter",
                                                                                                                                                                                                                                                source.getMetric().getCounter());
            XContentBuilder builder = jsonBuilder().prettyPrint().startObject().field("type",
                                                                                      "jdbc").field("jdbc").map(settingsBuilder.build().getAsStructuredMap()).endObject();
            String lastestState = builder.string();
            boolean success = syncTaskDTO.getSyncStateDTO().saveOrUpdate(builder.string());
            if (success) {
                logger.info("state persisted to database, {}", lastestState);
            } else {
    ...
    protected S createSource() {
        ...
        source.setUrl(url).setUser(user).setPassword(password).setLocale(LocaleUtil.toLocale(locale)).setTimeZone(TimeZone.getTimeZone(timezone));
        return source;
    }
