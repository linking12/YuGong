...
public class JDBCImporter extends AbstractPipeline<SettingsPipelineRequest> implements Runnable, CommandLineInterpreter {
    ...
    public JDBCImporter setSettings(Settings newSettings) {
        ...
        if (this.syncTaskDTO != null) {
            Settings stateSetting = this.syncTaskDTO.getSyncStateDTO().getConfigSetting();
    ...
    public JDBCImporter reloadSettings(Settings oldSettings) {
        if (this.syncTaskDTO != null) {
            Settings stateSetting = this.syncTaskDTO.getSyncStateDTO().loadConfigSetting();
            if (stateSetting != null) {
                settings = stateSetting;
            }
        }
        ...
        // if (statefile != null) {
        // try {
        // String newStatefile = FilePathUtil.buildPath(index, type, statefile);
        // File file = new File(newStatefile);
        // if (file.exists() && file.isFile() && file.canRead()) {
        // InputStream stateFileInputStream = new FileInputStream(file);
        // settings = settingsBuilder().put(oldSettings).loadFromStream("statefile",
        // stateFileInputStream).build();
        // logger.info("reloaded state from {}, settings {} ", newStatefile, settings.getAsMap());
        // } else {
        // logger.warn("can't read from {}, skipped", newStatefile);
        // }
        // } catch (IOException e) {
        // logger.error(e.getMessage(), e);
        // }
    ...
    @Override
    public void run(SyncTaskDTO syncTaskDto) {
        this.syncTaskDTO = syncTaskDto;
        String setting = syncTaskDto.getSetting();
        setSettings(settingsBuilder().loadFromSource(setting).build());
        run();
    }
    ...
    public void run() {
        try {
            ...
            try {
                ...
                SyncTaskStateDTO statedDTO = syncTaskDTO.getSyncStateDTO().loadLatestState();
                if (statedDTO.getIsCanceled()) {
                    shutdown = true;
                }
            } catch (Throwable e) {
    ...
    private List<Future> schedule(Settings settings) {
        ...
        if (schedule != null && schedule.length > 0) {
            ...
            CronThreadPoolExecutor cronThreadPoolExecutor = new CronThreadPoolExecutor(settings.getAsInt("threadpoolsize",
                                                                                                         1));
            ...
            Thread thread = new Thread(this);
            ScheduledThreadPoolExecutor scheduledThreadPoolExecutor = new ScheduledThreadPoolExecutor(settings.getAsInt("threadpoolsize",
                                                                                                                        1));
